...
disp(sprintf('%g \t\t %0.10f \t\t %0.10f\t\t %g', x0,root, ...
                lib.the_root - root, it_count));
...
%%
% a. Newton is much better at finding the root when the initial guess 
% is close to the root.

%%
% b. The accuracy of the estimation surpases the required accuracy because 
% on the last iteration Newton makes the error very small very fast, 
% surpassing the required accuracy. Newton converges to the root
% at a quadratic rate (x^2), so ending with the accuracy at 10^-12 rather
% than 10^-6 makes sense: i.e. (10^-6)^2 == (10^-12)

%%
% c. You can calculate the number of iterations by observing:
%    (1/2)^n * 1,000,000 <= 1.3x10^-12
% Solving for n will give you the number of iterations. In this case n
% comes out to be 60. Newton takes 67 iterations, so in this case 
% bisection is better.
...
%%
% a. When the interval is close to the root secant is better than bisection
% (by 11 iterations) and worse than Newton (by 4 iterations).

%%
% b. The initial size of the interval doesn't effect secant as much as it
% does bisection. Secant is very simliar to Newton; secant uses derivative
% information. It accelerates toward the root super-linearly so will make
% up for a large initial interval very quickly.

%% Problem 4
% The errors in the roots start to become large after roots function tries
% to evaluate the roots higher than 7. This happens because the
% polynomial evaluation at the higher roots yeilds very large numbers and
% as the approximate root get's close to the actual root you start to loose
